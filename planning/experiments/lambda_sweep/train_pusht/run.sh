...
echo "Launching Push-T inverse lambda sweep run: loss.inverse.weight=$LAMBDA_INV subdir=$LAMBDA_LABEL"
python -u "$REPO_ROOT/train.py" \
    "${CONFIG_ARGS[@]}" \
    "loss.inverse.weight=$LAMBDA_INV" \
    "subdir=$LAMBDA_LABEL" \
    "wandb.config.name=lambda_sweep_train_pusht_${LAMBDA_LABEL}"
