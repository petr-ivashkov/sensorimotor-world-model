...
if selected_goal_offset:
    ...
    if selected_goal_offset not in goal_offsets:
        raise SystemExit(f"goal_offset_steps={selected_goal_offset} is not in sweep.goal_offset_steps")
...
if not rows:
    raise SystemExit(f"No sweep entries matched run_name={selected_run!r} goal_offset_steps={selected_goal_offset!r}")
